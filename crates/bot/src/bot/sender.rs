use teloxide::{prelude::*, types::LinkPreviewOptions, types::ParseMode};
use tracing::warn;
...
///
/// Feed broadcasts ship Telegram HTML (`ParseMode::Html`) generated by
/// `crate::bot::html_format`; the sender always uses HTML so callers don't
/// need to remember to set a parse mode. Callers that don't need formatting
/// can pass plain text — Telegram accepts plain text in `sendMessage` without
/// a `parse_mode` (or with HTML, in which case no tags are interpreted).
    ...
    async fn send_text(
        ...
        match result {
            ...
            Err(err) => {
                ...
                // Telegram rejects malformed HTML with messages like
                // "Bad Request: can't parse entities" — log a truncated body
                // snippet so a bad feed can be diagnosed from the logs.
                if message.contains("parse entities") || message.contains("parse") {
                    let preview: String = text.chars().take(200).collect();
                    warn!(
                        chat_id,
                        error = %message,
                        body_preview = %preview,
                        "send failed to parse message entities; check feed HTML"
                    );
                }
